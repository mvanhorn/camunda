...
class TransformerVersionReplayDeterminismTest {
  ...
  void shouldSelectHandlerVersionPerStoredMap() {
    // given — a catalog whose SIGNAL v2 is the counting delegate
    final var v2Runs = new AtomicInteger();
    final var catalog =
        VersionedTransformerCatalog.builder()
            .register(TransformerSlot.SIGNAL, 2, ctx -> new CountingSignalTransformer(v2Runs))
            .build();
    ...
    // when — transform once pinned to SIGNAL v2, once with no versions (v1)
    new BpmnTransformer(
            el(),
            EngineConfiguration.DEFAULT_MAX_NAME_FIELD_LENGTH,
            catalog,
            Map.of(TransformerSlot.SIGNAL, 2))
        .transformDefinitions(model);
    final int afterV2 = v2Runs.get();
    new BpmnTransformer(el(), EngineConfiguration.DEFAULT_MAX_NAME_FIELD_LENGTH, catalog, Map.of())
        .transformDefinitions(model);
